...
-- Literals
---------------------------------------------------------------
L['%s will be disabled the next time the interface is reloaded.'] = '%s은(는) 다음에 인터페이스를 다시 불러올 때 비활성화됩니다.';
...
L['Friendly Players Only'] = '아군 플레이어만';
...
L['Navigate the interface with the gamepad using a virtual cursor.'] = '가상 커서를 사용해 게임패드로 인터페이스를 탐색합니다.';
...
L['Only open the unit menu when interacting with friendly players.'] = '아군 플레이어와 상호작용할 때만 유닛 메뉴를 엽니다.';
...
L['Radial on-screen keyboard for typing with the gamepad.'] = '게임패드로 입력할 수 있는 방사형 화면 키보드입니다.';
...
L['Replaces the default action bars with a layout designed for gamepad play.'] = '기본 행동 단축바를 게임패드에 맞게 설계된 배치로 대체합니다.';
...
L['Replaces the game menu with a controller-friendly menu and a quick access ring.'] = '게임 메뉴를 컨트롤러 친화적인 메뉴와 빠른 접근 원형 메뉴로 대체합니다.';
...
L['Switch the action bar layout to %s? Your current layout will be replaced.'] = '행동 단축바 배치를 %s(으)로 전환할까요? 현재 배치가 대체됩니다.';
...
L['Targeting tools: raid cursor, unit hotkeys and the target ring.'] = '대상 지정 도구: 공격대 커서, 유닛 단축키, 대상 원형 메뉴.';
...
L['Utility rings for spells, items and macros, selected with the radial stick.'] = '주문, 아이템, 매크로를 위한 유틸리티 원형 메뉴로, 방사형 스틱으로 선택합니다.';
...
L['World'] = '월드';
L['World interaction helpers: quick menu, loot frame and temporary ability prompts.'] = '월드 상호작용 도우미: 빠른 메뉴, 전리품 창, 임시 능력 안내.';
L['X Offset'] = 'X 오프셋';
