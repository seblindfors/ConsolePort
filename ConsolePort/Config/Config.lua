...
function ConsolePort:CreateIndicator(parent, size, anchor, button)
	...
	o:SetPoint("TOPLEFT", f, db.GUIDE["BORDER_X_"..size], db.GUIDE["BORDER_Y_"..size]);
	o:SetWidth(db.GUIDE["BORDER_S_"..size]);
	o:SetHeight(db.GUIDE["BORDER_S_"..size]);
	t:SetTexture(db.TEXTURE[button]);
	t:SetAllPoints(f);
	f:SetPoint(anchor, parent, db.GUIDE["BUTTON_"..anchor.."_"..size.."_X"], db.GUIDE["BUTTON_"..anchor.."_"..size.."_Y"]);
	f:SetWidth(db.GUIDE["BUTTON_S_"..size]);
	f:SetHeight(db.GUIDE["BUTTON_S_"..size]);
	f:SetAlpha(1);
