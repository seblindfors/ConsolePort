...
function ConsolePort:InitializeTalents()
	-- Indicators
	self:CreateIndicator(PlayerTalentFrameActivateButton, "SMALL", "RIGHT", db.NAME.CP_R_RIGHT);
	self:CreateIndicator(PlayerTalentFrameSpecializationLearnButton, "SMALL", "LEFT", db.NAME.CP_R_LEFT);
	self:CreateIndicator(PlayerTalentFrameTalentsLearnButton, "SMALL", "LEFT", db.NAME.CP_R_LEFT);
	-- Spec stuff
	GuideSpec1 = self:CreateIndicator(PlayerSpecTab1, "SMALL", "RIGHT", db.NAME.CP_R_UP);
	GuideSpec2 = self:CreateIndicator(PlayerSpecTab2, "SMALL", "RIGHT", db.NAME.CP_R_UP);
...
--			local target = _G[talents[talent_iterator]:GetName().."Selection"];
			...
			if IsShiftKeyDown() then
				...
				GameTooltip:AddLine(db.CLICK.TALENT, 1,1,1);
				GameTooltip:Show();
			else
