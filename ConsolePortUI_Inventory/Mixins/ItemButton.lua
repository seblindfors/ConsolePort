...
function Item:OnFocusGained()
	...
	if(speciesID and speciesID > 0) then
		ContainerFrameItemButton_CalculateItemTooltipAnchors(self, tooltip)
	...
	if ( requiresCompareTooltipReanchor ) then --and (IsModifiedClick("COMPAREITEMS") or GetCVarBool("alwaysCompareItems")) ) then
	...
	if texture then
		...
		if MerchantFrame:IsVisible() then
			if noValue then
				...
				Control:AddHint(KEY.CROSS, 'Sell')
			end
		else
